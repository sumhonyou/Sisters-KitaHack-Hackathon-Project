...
class _SosScreenState extends State<SosScreen>
  ...
  Future<void> _triggerEmergency() async {
    ...
    final msg = _sosService.generateEmergencyMessage(
      name: profile?['name'] ?? 'Unknown User',
      phone: profile?['phoneNumber'] ?? 'Unknown Phone',
      lat: lat,
      lng: lng,
      caseId: caseId,
    );
    ...
    if (lat != null && lng != null) {
      areaName = await _sosService.getAreaName(lat, lng);
    }
    ...
    if (mounted) {
      setState(() {
        ...
        _areaName = areaName;
        _caseId = caseId;
        _emergencyMessage = msg;
        _initializing = false;
      });
      _animateStatuses(); // Start the status animations once data is loaded
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final shortCase = _caseId != null
        ? 'EMG-${_caseId!.substring(0, 6).toUpperCase()}'
        : '';
    final locationText =
        _areaName ??
        ((_lat != null && _lng != null)
            ...
            children: [
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: () {
                    Clipboard.setData(ClipboardData(text: _emergencyMessage));
